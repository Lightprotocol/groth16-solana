...
// `groth16_solana::vk::gnark::generate_bsb22_vk_file` against
...
// `Setup` + `Prove` call for the variant-1 `LookupsCircuit` with X=7. gnark's
...
fn main() {
    ...
    for (src, _dst) in map.iter() {
        println!("cargo:rerun-if-changed=../fixtures/bsb22/{}", src);
    }
}
