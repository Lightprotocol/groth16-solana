...
mod tests {
    ...
    pub const PROOF: [u8; 256] = [
        ...
        149, 162, 44, 133, 120, 184, 205, 12, 44, 175, 98, 168, 44, 20, 24, 216, 15, 209, 175, 106,
        75, 147, 236, 90, 101, 123, 219, 245, 151, 209, 202, 218, 104, 148, 8, 32, 254, 243, 191,
        218, 122, 42, 81, 193, 84, 40, 57, 233, 205, 180, 46, 35, 111, 215, 5, 23, 93, 12, 71, 118,
        225, 7, 46, 247, 147, 47, 130, 106, 189, 184, 80, 146, 103, 141, 52, 242, 25, 0, 203, 124,
        176, 110, 34, 151, 212, 66, 180, 238, 151, 236, 189, 133, 209, 17, 137, 205, 183, 168, 196,
        92, 159, 75, 174, 81, 168, 18, 86, 176, 56, 16, 26, 210, 20, 18, 81, 122, 142, 104, 62,
        251, 169, 98, 141, 21, 253, 50, 130, 182, 15, 33, 109, 228, 31, 79, 183, 88, 147, 174, 108,
        4, 22, 14, 129, 168, 6, 80, 246, 254, 100, 218, 131, 94, 49, 247, 211, 3, 245, 22, 200,
        177, 91, 60, 144, 147, 174, 90, 17, 19, 189, 62, 147, 152, 18, 41, 139, 183, 208, 246, 198,
        118, 127, 89, 160, 9, 27, 61, 26, 123, 180, 221, 108, 17, 166, 47, 115, 82, 48, 132, 139,
        253, 65, 152, 92, 209, 53, 37, 25, 83, 61, 252, 42, 181, 243, 16, 21, 2, 199, 123, 96, 218,
        151, 253, 86, 69, 181, 202, 109, 64, 129, 124, 254, 192, 25, 177, 199, 26, 50,
    ];

    #[test]
    fn compression_syscall() {
        ...
        let proof_a = decompress_g1(&change_endianness(&PROOF[0..32]).try_into().unwrap()).unwrap();
        assert_eq!(proof_a.to_vec(), change_endianness(&PROOF[0..64]).to_vec());
        let index = 63;
        ...
        let proof_b = decompress_g2(&new_proof_b_bytes.try_into().unwrap()).unwrap();
        assert_eq!(
            proof_b.to_vec(),
        ...
        let mut new_proof_c_bytes = PROOF[192..].to_vec();
        ...
        let proof_c = decompress_g1(&new_proof_c_bytes[0..32].try_into().unwrap()).unwrap();
